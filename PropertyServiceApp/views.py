            ...
            if request.method == "POST":
                ...
                if not Users.objects.filter(phone_number=data["phone_number"]).exists():
                    ...
                    if verification.status == 'approved':
                        ...
                        user = Users.objects.create(phone_number=phone_number, full_name=full_name, email=email, address=address)
    ...
    if request.method == 'POST':
        ...
        if user :
            ...
            if send_otp:
                ...
                context = {"message" : "OTP SENT TO REGISTERED MOBILE NUMBER..."}
                return Response(context)
        else:
            context = {"message" : "User Doesn't Exist, Go to Registeration..."}
            return Response(context)
...
def verify_login(request):
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            return Response(context)
        if user :
            verification = verification_checks(data["phone_number"], data["otp"])
